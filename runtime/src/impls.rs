use crate::{AccountId, PropertyAssets, Runtime};
...
impl HandleCredit<AccountId, PropertyAssets> for CreditToBlockAuthor {
	fn handle_credit(credit: Credit<AccountId, PropertyAssets>) {
		//		fn handle_credit(credit: CreditOf<AccountId, Assets>) {
		if let Some(author) = pallet_authorship::Pallet::<Runtime>::author() {
			// Drop the result which will trigger the `OnDrop` of the imbalance in case of error.
			let _ = PropertyAssets::resolve(&author, credit);
		}
	}
}
